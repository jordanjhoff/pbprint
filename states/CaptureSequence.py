...
#TODO major refactor of this state required at some point.
class CaptureSequence(State):
    ...
    def send_job(self, photo_output_path=None) -> bool:
        ...
        create_photo(
            ...
            vertical_pad= self.config.VERTICAL_PADDING,
            horizontal_pad= self.config.HORIZONTAL_PADDING,
    ...
    def next_state(self, *args) -> State:
        ...
        if not self.send_job(f"{output_dir}/final_photo.png"):
            return DisplayText(
                ...
                timeout=10,
                next_state_lambda=(lambda: Start(self.state_manager, config=self.config)),)

        final = lambda: DisplayText(
            state_manager=self.state_manager,
            config=self.config,
...
def add_padding(image, horizontal_pad, vertical_pad, horizontal_shift, vertical_shift):
    """
    Adds some padding around the image before printing.
    :param image:
    :param horizontal_pad:
    :param vertical_pad:
...
def create_photo(
    ...
    """
    Creates the final image and saves it to the specified path.
    :param image_paths:
    :param template:
    :param output_path:
    :param horizontal_pad:
